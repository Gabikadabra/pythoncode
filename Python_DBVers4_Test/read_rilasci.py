...
#Separa i rilasci che non risultano pià attivi
wRil_Status = ['Chiusura Amministrativa', 'Concluso', 'Sospeso','Annullato']
df_Rilasci_subt_act=df_Rilasci.loc[~df_Rilasci["Stato Incarico"].isin(wRil_Status)&(df_Rilasci["Codice Cliente"].str.contains("5000039", na=False)),["INCARICO","Codice Rilascio Contr."]]
wIncarichi_list_act=df_Rilasci_subt_act["INCARICO"].unique()
wRilasci_list_act=df_Rilasci_subt_act["Codice Rilascio Contr."].unique()

df_Rilasci_subt=df_Rilasci.groupby(["Tipo Incarico", "Codice Rilascio Contr.", "Titolo Rilascio Contr.", "Tipo Rilascio", "Linea di business", "INCARICO","Stato Incarico"])[["BDG dei costi Corrente","BDG dei costi Contrattuale", "Consuntivi Totali"]].sum().reset_index()
#Rilasci Demand
...
df_Rilasci_task=df_Rilasci.groupby(["Tipo Incarico", "Codice Rilascio Contr.", "Titolo Rilascio Contr.", "Tipo Rilascio", "Linea di business", "INCARICO","Master Program",	"Demand",	"Profit Center MP",	"Descrizione Area MP","Numero Progetto",	"Nome Progetto",	"Stato Progetto",	"Responsabile Progetto",	"Profit Center Progetto",	"Descrizione Area Progetto",	"Tipologia Progetto",	"Numero Task",	"Nome Task",	"Tipo Task", "Stato Incarico"]).agg(BDGCostiCorrenti=("BDG dei costi Corrente",np.sum),BDGCostiContratt=("BDG dei costi Contrattuale",np.sum),ConsTotali=("Consuntivi Totali",np.sum),DataIniz=("Data inizio Attività Ver. Corrente",np.min),DataFine=("Data di Fine Attività Ver. Corrente",np.max),DataValidRC=("Data Validazione RC",np.max)).reset_index()
...
#for specific INCARICO's map new dept name
#w_list_eccez_IncarichiDip = [['DIH', '23SF4'],['DIH', '24SF4'],['DIH', '25SF4']]
#Create dataframe
#w_eccez_Rilasci = pd.DataFrame(w_list_eccez_IncarichiDip, columns=['eccez_Dip', 'INCARICO'])
df_Rilasci_task=pd.merge(df_Rilasci_task,w_eccez_Rilasci,on="INCARICO",how="left",suffixes=('', '_pos'))
df_Rilasci_task["resp_dmd_dip"]= np.where(df_Rilasci_task['eccez_Dip'].notnull(),df_Rilasci_task['eccez_Dip'],df_Rilasci_task['resp_dmd_dip'])
...
df_Rilasci_subt=pd.merge(df_Rilasci_subt,w_eccez_Rilasci,on="INCARICO",how="left",suffixes=('', '_pos'))
df_Rilasci_subt["resp_dmd_dip"]= np.where(df_Rilasci_subt['eccez_Dip'].notnull(),df_Rilasci_subt['eccez_Dip'],df_Rilasci_subt['resp_dmd_dip'])
...
logger.info('end reading rilasci ' + str(df_Rilasci_subt.shape[0]))
