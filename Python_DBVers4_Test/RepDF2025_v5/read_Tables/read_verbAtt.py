...
df_elab_Verb_Spalm=df_RCA_VerbSpalm.groupby(["cod verbale","cod rilascio", "num impegno",	"anno impegno",	"num capitolo",	"anno capitolo",	"num decreto",	"anno_decreto","mensilita"]).agg(importo_verbxcap= ("importo_verbxcap","sum")).reset_index()
df_elab_Imp_Spalm=df_RCA_ImpSpalm.groupby(["num impegno", "anno impegno", "cod rilascio", "num capitolo", "anno capitolo", "num decreto", "anno_decreto"]).agg(**{"importo impegno": ("importo impegno","sum")}).reset_index()


df_Elab_RCA_VerbSpalm=df_RCA_VerbSpalm.groupby(["cod verbale","cod rilascio", "Rif_Impegno", "Rif_Capitolo", "Rif_Decreto", "Ril_Finanz"]).agg(Importo_verbxcap= ("importo_verbxcap","sum")).reset_index()
...
df_elab_Imp_Spalm=df_elab_Imp_Spalm.loc[df_RCA_ImpSpalm["cod rilascio"].isin(wRilasci_cln_list)]
...
df_elab_RefRil=df_elab_RefRil.loc[df_elab_RefRil["cod incarico"].isin(wIncarichi_cln_list)]
df_elab_RefRil["Ordine approvazione"]=df_elab_RefRil["Ordine approvazione"].astype(str)

tempoFin = timeit.default_timer()
logger.info('end reading ' + str(df_elab_RefRil.shape[0])+" Durata processo: " + wPythProc +" - dim: "+str(df_elab_RefRil.memory_usage().sum())+" Durata processo: "  + str(round(tempoFin-tempoInz,3))+ " Dataframe dimensione " + str(df_elab_RefRil.memory_usage(deep=True)) )
