...
w_colnumeric_toclean=["QTA Prev. Corr.",'QTA Cons. di Periodo','QTA YTD']
for w_colnamecln in w_colnumeric_toclean:
	df_prog[w_colnamecln]=df_prog[w_colnamecln].apply(remove_chars);df_prog[w_colnamecln]=df_prog[w_colnamecln].apply(pd.to_numeric,errors="coerce").fillna(0)


"""#begin format columns - reduce size"""
#wLi=["CID"];wDi={value:"int64" for value in wLi}
#wLs=["Prestazione"];wDs={value:"str" for value in wLs}
wLf=["QTA Prev. Corr.","QTA Prev. Contr.","Imp. Prev. Corr.","Imp. Prev. Contr.","QTA Cons. di Periodo","QTA YTD","Imp. Cons. di Period","Imp. Cons. YTD"];wDf={value:"float" for value in wLf}
wLc=["Cod. Area Progetto","Task ID","Tipologia Progetto","Tipo Rilascio","Stato Progetto","Cod. Area MP","Codice MP","Task Type","Categoria Risorsa","Codice Risorsa","Codice RL","Descrizione RL","UdM","Descrizione Risorsa","Linea di business","Incarico","Divisione","Cod. Lotto di Gara","Descr. Lotto di Gara","Cod. Macroclasse","Descr. Macroclasse","cod. Area Task","Cdc Supervisore Task","CUP"];wDc={value:"category" for value in wLc}
wLnbr=wLf
df_prog[wLnbr]=df_prog[wLnbr].fillna(0)
...
df_prog= df_prog.astype(convert_dict)
...
"""#end format columns - reduce size"""
if (wrk_environm.lower()=="testfilter") & ("*all" not in listfilter_Task):
    df_prog=df_prog.loc[(df_prog["Codice Task"].isin(listfilter_Task))]
...
df_prog["CID"]= np.where((df_prog["Categoria Risorsa"]=="Fogli Ore"),df_prog["Codice Risorsa"].str.slice(3, 6),0 ) 
#df_progaA= df_prog.loc[df_prog["Categoria Risorsa"]=="Fogli Ore"] 
#df_progaA["CID"]= df_prog["Codice Risorsa"].str.slice(3, 3) 

df_prog= df_prog.astype({"CID":int})
#Assegna il tipo risorsa sulla base della categoria
df_prog["TipoRis"]= df_prog['Categoria Risorsa'].map(dict_RscType)
df_prog=pd.merge(df_prog,df_RisInt_byName[["RisInt_nome","CdC","Int_Dip"]],left_on="Resp. Progetto", right_on="RisInt_nome",how="left",suffixes=('', '_prog'))

df_prog=pd.merge(df_prog,df_RisInt_byName[["RisInt_nome","CdC","Int_Dip"]],left_on="Resp. MP", right_on="RisInt_nome",how="left",suffixes=('', '_MP'))
df_prog=pd.merge(df_prog,df_RisInt_byCID[["CID","RisInt_nome","CdC","Int_Dip"]],left_on="CID", right_on="CID",how="left",suffixes=('', '_int'))
df_prog=df_prog.rename(columns={"CdC_MP":"MP_CdC","Int_Dip_MP":"MP_Dip" ,"CdC_int":"RisInt_CDC","RisInt_nome":"RisInt" ,"Resp. Progetto":"Resp. PM","Int_Dip":"PM_Dip","Int_Dip_int":"RisInt_Dip","Linea di business":"LOB" })
...
#df_progtask= df_prog.groupby(["Cod. Area Progetto","Cod. Progetto","Descrizione Progetto","Tipologia Progetto","Tipo Rilascio","Stato Progetto","Resp. PM","Resp. MP","Cod. Area MP","Codice MP","Progetto CdF","Codice Task","Riferimento WBS","Task Type","Incarico","Descrizione Task","CUP","TipoRis","CdC","MP_CdC","MP_Dip","PM_Dip"],observed=True,as_index=False).agg({"Data I. Corrente":"min","Data F. Corrente":"max","Data I. Contrattuale":"min","Data F. Contrattuale":"max"})
...
#df_prog_ammort=df_prog_ammort.fillna(0)
#recalculate task with Ammortamenti
df_prog_ammort_grp=df_prog_ammort.groupby(
["Cod. Area Progetto","Cod. Progetto","Descrizione Progetto","Tipologia Progetto","Tipo Rilascio","Stato Progetto","Resp. PM","Resp. MP","Cod. Area MP","Codice MP","Progetto CdF","Codice Task","Task Type","Incarico","Data I. Corrente","Data F. Corrente","Data I. Contrattuale","Data F. Contrattuale","Anno Inizio","Anno Fine","Descrizione Task","CUP","CID","MP_CdC","MP_Dip","PM_Dip"],observed=True, dropna=False).agg(**{
...
df_prog_int_riep = df_prog_int.groupby(["RisInt_Dip","RisInt","Cod. Progetto","Codice Task"],dropna=False,observed=True).agg(
...
df_prog_byResCategory=df_prog.groupby(["Cod. Area Progetto","Cod. Progetto","Descrizione Progetto","Tipologia Progetto","Tipo Rilascio","Stato Progetto","Resp. PM","Resp. MP","Cod. Area MP","Codice MP","Progetto CdF","Codice Task","Riferimento WBS","Task Type","Categoria Risorsa","Codice RL","Descrizione RL","Incarico","Data I. Corrente","Data F. Corrente","Data I. Contrattuale","Data F. Contrattuale","Descrizione Task","CUP","CID","TipoRis","CdC","MP_CdC","MP_Dip","PM_Dip"],observed=True,as_index=False).agg(
...
["Cod. Area Progetto","Cod. Progetto","Descrizione Progetto","Tipologia Progetto","Tipo Rilascio","Stato Progetto","Resp. PM","Resp. MP","Cod. Area MP","Codice MP","Progetto CdF","Codice Task","Task Type","Incarico","Data I. Corrente","Data F. Corrente","Data I. Contrattuale","Data F. Contrattuale","Descrizione Task","CUP","CID","TipoRis","MP_CdC","MP_Dip","PM_Dip","LOB"],observed=True,as_index=False).agg(Pianif_Corr_QTA= ("QTA Prev. Corr.","sum"),
Pianif_Contr_QTA=("QTA Prev. Contr.","sum"),
Pianif_Corr =("Imp. Prev. Corr.","sum"),
Pianif_Contr=("Imp. Prev. Contr.","sum"),
Consuntivo_QTA=("QTA Cons. di Periodo","sum"),
...
["Cod. Area Progetto","Cod. Progetto","Descrizione Progetto","Tipologia Progetto","Tipo Rilascio","Stato Progetto","Resp. PM","Resp. MP","Cod. Area MP","Codice MP","Progetto CdF","Codice Task","Task Type","Incarico","Descrizione Task","CUP","MP_CdC","MP_Dip","PM_Dip","LOB"],observed=True,as_index=False,dropna=False).agg(**
{"Data I. Corrente":("Data I. Corrente","min"),"Data F. Corrente":("Data F. Corrente","max"),"Data I. Contrattuale":("Data I. Contrattuale","min"),"Data F. Contrattuale":("Data F. Contrattuale","max"),
"Pianif_Corr" :("Imp. Prev. Corr.","sum"),
"Pianif_Contr":("Imp. Prev. Contr.","sum"),
"Consuntivo":("Imp. Cons. di Period","sum"),
"ConsuntivoYTD":("Imp. Cons. YTD","sum")
